...
func (controller *repository) InitScan(c echo.Context) error {
	...
	defer lock.Unlock()
	name := c.Param("name")
	controller.repositoryService.AddScanDetails(name, model.QUEUED)
...
func (controller *repository) ExecuteScanner(c echo.Context) error {
	lock.Lock()
	defer lock.Unlock()
	err := controller.repositoryService.ExecuteScanner()
	if err != nil {
...
//	@Success	200		{string}	json			 "ok"
//	@Failure	400		{object}	string	 		 "We need name and url !!"
//	@Router		/repository/findings [get]
func (controller *repository) ListFindings(c echo.Context) error {
	lock.Lock()
	defer lock.Unlock()
	repos, _ := controller.repositoryService.ListFindings()
	return c.JSON(http.StatusOK, repos)
}
